# Corrected and complete main.tf
...
# 8. Create the IAM OIDC Provider for the cluster
...
# 9. Create an IAM role for the Node Group workers AND Jenkins Agents
resource "aws_iam_role" "eks_node_group_role" {
  name = "${var.cluster_name}-node-group-role"

  # --- THIS IS THE CORRECTED TRUST POLICY ---
  # It now contains TWO statements: one for the Jenkins agent (Federated)
  # and one for the EC2 nodes themselves (Service).
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${aws_iam_openid_connect_provider.eks.url}:sub" = "system:serviceaccount:jenkins:default"
          }
        }
      },
      {
...
resource "aws_iam_role_policy_attachment" "node_policy_ecr_read" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"
  role       = aws_iam_role.eks_node_group_role.name
}

resource "aws_iam_policy" "jenkins_ecr_policy" {
  name        = "${var.cluster_name}-JenkinsECRPolicy"
  description = "Allows Jenkins agents to create and push to ECR"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_eks_node_group" "dev_node_group" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.node_policy_ecr_read,
    aws_iam_role_policy_attachment.jenkins_ecr_attachment,
  ]
}
